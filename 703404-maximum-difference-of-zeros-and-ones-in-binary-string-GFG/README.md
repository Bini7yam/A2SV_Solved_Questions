...
Your task:You do not need to read any input or print anything. The task is to complete the function maxSubstring(), which takes a string as input and returns an integer. 
Expected Time Complexity:&nbsp;O(|S|)Expected Auxiliary Space: O(1)
Constraints:1 ≤ |S| ≤ 105S contains 0s and 1s only